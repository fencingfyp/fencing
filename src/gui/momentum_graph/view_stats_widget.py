...
class ViewStatsWidget(BaseTaskWidget):
    ...
    def on_runButton_clicked(self):
        ...
        try:
            ...
            start_frame = load_start_frame(self.working_dir)
        except FileNotFoundError as e:
            self.interactive_ui.write(str(e))
            return

        # Show momentum graph
        graph_np = render_momentum_graph(momentum_df, self.fps)
...
def load_start_frame(working_dir: str) -> int:
    path = os.path.join(working_dir, START_TIME_JSON_NAME)
    if not os.path.exists(path):
        raise FileNotFoundError("Start time file not found.")
    with open(path, "r") as f:
        return json.load(f)["start_frame"]
...
def compute_time_between_points(
    ...
    """
    Returns a reduced DataFrame with:
    - Momentum
    - Delta Time (sec)
    First row is excluded.
    """
    ...
    # Drop first row and keep only relevant columns
...
def render_momentum_graph(momentum_df: pd.DataFrame, fps: float) -> np.ndarray:
