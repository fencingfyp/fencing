...
class Ui:
    ...
    def get_quadrilateral(
        self, item_name: str, return_original_size: bool = False
    ) -> Quadrilateral:
        return Quadrilateral(
            self.get_n_points(
                generate_select_quadrilateral_instructions(item_name),
                return_original_size,
            )
        )

    def get_n_points(
        self, instructions: list[str], return_original_size: bool = False
    ) -> list[tuple[int, int]]:
