...
def main():
    ...
    with open(output_csv_path, mode, newline="") as f:
        ...
        while True:
            ...
            is_left_red = left_colour_detector.check_light(
                ui.get_current_frame(), left_score_positions
            )
            is_right_green = right_colour_detector.check_light(
                ui.get_current_frame(), right_score_positions
            )

            if not demo_mode:
                csv_writer.writerow(
                    [frame_id, 1 if is_left_red else 0, 1 if is_right_green else 0]
                )
