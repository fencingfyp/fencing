...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Extract score increases from a smoothened graph."
    )
...
def main():
    ...
    smoothed_df = pd.DataFrame(
        {
            "frame_id": np.arange(len(pred)),
            "left_score": _retroactive_flatten(pred[:, 0].tolist()),
            "right_score": _retroactive_flatten(pred[:, 1].tolist()),
        }
    )
    smoothed = densify_frames(smoothed_df, len(smoothed_df)).to_numpy()
